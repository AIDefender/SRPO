...
def algo_init(args):
    ...
    sa_prior = Net(layer_num=args['hidden_layers'], 
                  state_shape=obs_shape+action_shape, 
                  output_shape=1,
                  hidden_layer_size=args['hidden_layer_size']).to(args['device'])
    sa_prior_optim = torch.optim.Adam(sa_prior.parameters(), lr=args['sa_prior_lr'])
    
    sas_prior = Net(layer_num=args['hidden_layers'], 
                  state_shape=obs_shape*2+action_shape, 
                  output_shape=1,
                  hidden_layer_size=args['hidden_layer_size']).to(args['device'])
    sas_prior_optim = torch.optim.Adam(sas_prior.parameters(), lr=args['sa_prior_lr'])
    ...
    return {
        ...
        "sa_prior": {"net": sa_prior, "opt": sa_prior_optim},
        "sas_prior": {"net": sas_prior, "opt": sas_prior_optim},
    }
...
class AlgoTrainer(BaseAlgo):
    def __init__(self, algo_init, args):
        ...
        self.sa_prior = algo_init['sa_prior']['net']
        self.sa_prior_optim = algo_init['sa_prior']['opt']
        self.sas_prior = algo_init['sas_prior']['net']
        self.sas_prior_optim = algo_init['sas_prior']['opt']
    ...
    def train(self, train_buffer, val_buffer, callback_fn):
        ...
        for i in range(self.args['out_train_epoch']):
            ...
            for j in range(self.args['in_train_epoch']):
                batch, source_batch, target_batch = self.get_train_policy_batch(self.args['train_batch_size'])
                in_res = self.train_policy(batch, source_batch, target_batch)
                for key in in_res:
    ...
    def get_train_policy_batch(self, batch_size = None):
        ...
        source_indice = np.random.randint(0, 
                                          self.args['num_origin']//self.args['horizon'], 
                                          env_batch_size//2)
        target_indice = np.random.randint(self.args['num_origin']//self.args['horizon'], 
                                          self.env_pool._size, 
                                          env_batch_size//2)
        
        source_batch = self.env_pool.batch_by_indices(source_indice)
        target_batch = self.env_pool.batch_by_indices(target_indice)
        ...
        if model_batch_size > 0:
            ...
            batch = env_batch
        return batch, source_batch, target_batch
    ...
    def train_policy(self, batch, source_batch, target_batch):
        batch['valid'] = batch['valid'].astype(int)
        lens = np.sum(batch['valid'], axis=1).squeeze(-1)
        max_len = np.max(lens)
        for one_batch in [batch, source_batch, target_batch]:
            for k in one_batch:
                one_batch[k] = torch.from_numpy(one_batch[k][:,:max_len]).to(self.device)
        ...
        with torch.no_grad():
            ...
            if self.args['rew_reg_eta'] > 0.:
                # todo: compute is for transition
                obs = batch['observations'].reshape(-1, self.obs_space.shape[0])
                act = batch['actions'].reshape(-1, self.action_space.shape[0])
                next_obs = batch['next_observations'].reshape(-1, self.obs_space.shape[0])

                sa_prior = self.sa_prior(torch.cat([obs, act], dim=1))[0].reshape(-1, max_len, 1)
                sas_prior = self.sas_prior(torch.cat([obs, act, next_obs], dim=1))[0].reshape(-1, max_len, 1)
                Q_target = Q_target + self.args['rew_reg_eta'] * torch.log(torch.clamp(sas_prior-sa_prior, 0.1 * torch.ones_like(sa_prior), 10 * torch.ones_like(sa_prior)))
        ...
        s_prior_loss = 0
        # obtain sa and sas loss
        for index in [0, 1]:
            if index == 0: 
                train_batch = source_batch
                label_func = torch.zeros
            if index == 1: 
                train_batch = target_batch
                label_func = torch.ones
            obs = train_batch['observations'].reshape(-1, self.obs_space.shape[0])
            act = train_batch['actions'].reshape(-1, self.action_space.shape[0])
            next_obs = train_batch['next_observations'].reshape(-1, self.obs_space.shape[0])
            
            sa_batch = torch.cat([obs, act], 1)
            sas_batch = torch.cat([obs, act, next_obs], 1)
            loss = torch.nn.BCELoss()
            label = label_func(obs.shape[0], 1).to(device=self.args['device'])
            sas_loss = loss(torch.sigmoid(self.sas_prior(sas_batch)[0]), label)
            sa_loss = loss(torch.sigmoid(self.sa_prior(sa_batch)[0]), label)
            
            self.sa_prior_optim.zero_grad()
            self.sas_prior_optim.zero_grad()
            sas_loss.backward()
            sa_loss.backward()
            self.sa_prior_optim.step()
            self.sas_prior_optim.step()

            s_prior_loss += sas_loss.item() + sa_loss.item()
